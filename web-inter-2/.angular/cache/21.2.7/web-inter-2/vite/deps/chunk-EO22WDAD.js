...
var y2 = class extends b {
  ...
  async getAttachments() {
    ...
    const c7 = i3?.map((t3) => `${t3.field} ${"descending" === t3.order ? "DESC" : "ASC"}`), h5 = new p2({ objectIds: [o3], returnMetadata: true, orderByFields: c7, attachmentTypes: s8 ? a5?.filter(Boolean).map((t3) => `${t3}/*`) : void 0, keywords: r5 }), d7 = [], l5 = t2.queryAttachments(h5).then((t3) => t3[o3] || d7).catch(() => d7);
  ...
  async _queryAttachment(t2, e7) {
    ...
    const r5 = new p2({ objectIds: [a5], attachmentsWhere: `AttachmentId=${t2}`, returnMetadata: true });
...
var c4 = class extends b {
  constructor(t2) {
    super(t2), this._loadingPromise = null, this.created = null, this.creator = null, this.destroyer = null, this.graphic = null, this.addHandles(l(() => this.creator, (t3) => {
  ...
  _destroyContent() {
    ...
    t2 && e7 && (x2({ type: "content", value: r5, event: { graphic: e7 } }), this._set("created", null));
  }
  async _createContent(t2) {
    const e7 = this.graphic;
    if (!e7 || !t2) return;
    const r5 = x2({ type: "content", value: t2, event: { graphic: e7 } });
...
async function $2(e7, t2, r5, i3) {
  ...
  if (b7?.relationshipTableId && b7.keyFieldInRelationshipTable) {
    ...
    if (y5?.length && u5.supportsStatistics) {
      ...
      const r6 = { features: Promise.resolve(t3), statsFeatures: s3(p8, e8) };
      return j(r6);
    }
  }
  const g3 = b7?.keyField;
  if (g3) {
    const t3 = Fe2(L2(u5.fields, g3)), o4 = F(e7.attributes, c7.keyField), f11 = t3 ? `${g3}=${o4}` : `${g3}='${o4}'`, d8 = r5.historicMoment, I4 = r5.gdbVersion, b8 = s3(p8, new R({ where: f11, outFields: h5, sourceSpatialReference: m5, historicMoment: d8, gdbVersion: I4 }), i3), S4 = !!y5?.length && u5.supportsStatistics ? s3(p8, new R({ where: f11, outFields: h5, outStatistics: y5, sourceSpatialReference: m5 }), i3) : null, w6 = { features: b8 };
    return S4 && (w6.statsFeatures = S4), j(w6);
  }
  return null;
}
function N3(t2, r5) {
  return f3(t2, { query: { f: "json" }, signal: r5?.signal });
}
...
function q3({ relatedInfo: e7, fieldName: t2, fieldInfo: r5 }) {
  if (e7.relatedFields?.push(t2), r5.statisticType) {
    const i3 = new p4({ statisticType: r5.statisticType, onStatisticField: t2, outStatisticFieldName: t2 });
...
function v4(e7) {
  return e7.isAggregate ? "popup-feature-reduction" : "esri.views.3d.layers.VoxelGraphic" === e7.declaredClass ? "popup-voxel" : n4(e7.origin) || n5(e7.origin) ? "popup-imagery" : "popup";
}
function h3(e7) {
...
async function k() {
  const [e7, r5, { Feature: t2 }] = await Promise.all([import("./arcade-5KYO2L3B.js"), import("./arcade-RZC23HUZ.js"), import("./Feature-OJ5GXLYC.js")]);
...
var E2 = class extends b {
  constructor(e7) {
    ...
    }, this._compileThrottled = l4(this._startCompile, () => this.notifyChange("state"), k2, this), this._evaluateThrottled = l4(this._startEvaluate, () => this.notifyChange("state"), k2, this), this.addHandles([l(() => [this.expressionInfo, this.graphic], () => this._compileThrottled(), h), l(() => [this.contentElement], () => this._createVM(), h), f2(() => {
  ...
  async _queryFeatureCount() {
    ...
    if ("subtype-group" === t2?.type && !l5) {
      if (this._set("featureCount", 0), this._destroyRelatedFeatureViewModels(), this.featurePage = 1, this.relatedFeatures.destroyAll(), s8 && null != u5) {
        const { default: e8 } = await import("./uniqueValues-ASR2DJXN.js"), { uniqueValueInfos: l6 } = await e8({ layer: t2, sqlWhere: `${s8} = '${u5}'`, field: t2.subtypeField, signal: r5?.signal }), a6 = l6.map(({ count: e9, value: r6 }) => {
    ...
    const { historicMoment: y5, gdbVersion: h5 } = e7, c7 = new d3({ cacheHint: d7, gdbVersion: h5, historicMoment: y5, relationshipId: n8, returnGeometry: false, objectIds: [i3], where: this._getRelationshipWhereClause(t2) }), g3 = await e7.queryRelatedFeaturesCount(c7, { signal: r5?.signal });
...
var ne = class extends o2(b) {
  ...
  constructor(e7) {
    ...
    }, this._evaluateExpressionAttributesThrottled = l4(this._evaluateExpressionAttributes, () => this.notifyChange("waitingForContent"), ie, this), this.addHandles([l(() => [this.graphic, this._effectivePopupTemplate, this.abilities, this.timeZone], () => this._graphicChangedThrottled(), h), f2(() => {
  ...
  _compileContent(e7) {
    if (this._destroyContentViewModels(), this.graphic) return Array.isArray(e7) ? e7.filter(this._isAllowedContentType).map((e8, t2) => this._compileContentElement(e8, t2)).filter(N) : "string" == typeof e7 ? this._compileText(new n2({ text: e7 }), 0).text : e7;
  }
  ...
  _compileUtilityNetworkAssociation(e7, t2) {
    ...
    return this.contentViewModels[t2] = p8, this.addHandles([l(() => p8.associationFeatures.values(), () => this._setUpUtilityNetworkAssociationsViewModels(p8.associationViewModels, p8.associationFeatures, p8.map))], re), e7;
  }
  _compileExpression(e7, t2) {
    ...
    return this.contentViewModels[t2] = l5, this.addHandles(l(() => l5.contentElementViewModel, () => this._setExpressionContentVM(l5, t2), h), se2), e7;
  }
  ...
  async _getTitle() {
    ...
    return t2 ? x2({ type: "title", value: e7?.title, event: { graphic: t2 } }) : null;
  }
  async _getContent() {
    const { _effectivePopupTemplate: e7, graphic: t2 } = this;
    return t2 ? x2({ type: "content", value: e7?.content, event: { graphic: t2 } }) : null;
  }
...
//# sourceMappingURL=chunk-EO22WDAD.js.map

...
function g(t2, n, r2, o) {
  return b(t2, t2[0] + n, t2[1] + r2, null != t2[2] && null != o ? t2[2] + o : void 0);
}
...
function M2(t2, n, r2, o, e2) {
  const [i2, s, ...c3] = t2;
  return [(i2 - o) * n + o, (s - e2) * r2 + e2, ...c3];
}
function j2(t2, n, r2, o, e2, i2) {
  if (e(t2)) return M2(t2, n, r2, o, e2);
  if (i(t2)) {
    const [i3, s, c4] = t2.b.map((t3) => M2(t3, n, r2, o, e2));
    return { b: [i3, s, c4] };
  }
  if (!i2) return t2;
  const c3 = v(i2), y3 = c(t2) ? f(c3, t2) : t2, [x2, h3, g2, v3, d2, j3, U2] = y3.a, z2 = d2 ?? 0, b2 = j3 ?? Math.hypot(x2[0] - h3[0], x2[1] - h3[1]), P2 = U2 ?? 1, R = c2(o, e2, 1, 0, 0, 1, n, r2);
  return w(c3, { a: [[...x2], [...h3], g2, v3, z2, b2, P2] }, R);
}
function U(t2, n, r2, o, e2, i2) {
  const s = Math.sqrt((r2 - t2) * (r2 - t2) + (o - n) * (o - n));
  return Math.sqrt((e2 - t2) * (e2 - t2) + (i2 - n) * (i2 - n)) / s;
}
function z(t2, r2, o, e2 = false) {
  const i2 = Math.atan2(r2.y - o.y, r2.x - o.x) - Math.atan2(t2.y - o.y, t2.x - o.x), s = Math.atan2(Math.sin(i2), Math.cos(i2));
  return e2 ? s : M(s);
}
function b(t2, n, r2, o) {
  const e2 = [n, r2];
  return t2.length > 2 && e2.push(null != o ? o : t2[2]), t2.length > 3 && e2.push(t2[3]), e2;
}
function P(t2) {
  const n = t2?.getVertexCandidates();
  return n?.at(0)?.targetPoint.z;
}
...
//# sourceMappingURL=chunk-2GWIYL23.js.map

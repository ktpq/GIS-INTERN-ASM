import {
  p as p3
} from "./chunk-VHXV3Q5D.js";
import {
  a as a3,
  m as m2,
  p as p2,
  y
} from "./chunk-HANE3BNO.js";
import {
  _ as _2
} from "./chunk-DUE7DBV5.js";
import "./chunk-R5Z7TSMK.js";
import "./chunk-FLIHJ2WP.js";
import "./chunk-H5PQOJ5Z.js";
import {
  R,
  j,
  w
} from "./chunk-KWGDYBY5.js";
import "./chunk-DPFYRE7L.js";
import "./chunk-V2S3DWFP.js";
import "./chunk-HMACNZAH.js";
import "./chunk-6XOPOCWC.js";
import "./chunk-X2TALAHM.js";
import "./chunk-TZSKNWRI.js";
import "./chunk-DZ6LTFHJ.js";
import "./chunk-6FS257XK.js";
import {
  j as j2
} from "./chunk-QG6BYYHC.js";
import "./chunk-FBMIFYPT.js";
import "./chunk-UGNCDVPZ.js";
import "./chunk-QJORC5MD.js";
import "./chunk-3LIHIPQ4.js";
import {
  s as s2
} from "./chunk-HWUPLHAX.js";
import "./chunk-SQCKII37.js";
import {
  s
} from "./chunk-6LOCYJ7K.js";
import "./chunk-DPKT4LYX.js";
import {
  l as l3
} from "./chunk-SAIKS64R.js";
import {
  _
} from "./chunk-BZNIQO2Y.js";
import "./chunk-OTOTHRZE.js";
import "./chunk-YK5DPRSK.js";
import "./chunk-7HUYAIOT.js";
import "./chunk-ERWWBZRX.js";
import "./chunk-LBXDB7YQ.js";
import "./chunk-5L27TA3I.js";
import "./chunk-5TEEMD4W.js";
import "./chunk-MESRIVTQ.js";
import "./chunk-TDVHS7EW.js";
import {
  l as l2
} from "./chunk-LPNV4GQD.js";
import {
  g
} from "./chunk-AUNLF36X.js";
import {
  e
} from "./chunk-T3TXTZDJ.js";
import {
  m
} from "./chunk-PX67TG6P.js";
import "./chunk-IJHRPSGC.js";
import {
  b
} from "./chunk-DVV7T3XR.js";
import "./chunk-Z5NNQZAE.js";
import "./chunk-FLG5AFOF.js";
import "./chunk-OKJ7MV5V.js";
import "./chunk-23U7MZU6.js";
import "./chunk-NNUIV2NH.js";
import "./chunk-JL3Z5SD5.js";
import "./chunk-IANKUBS7.js";
import "./chunk-IZTHTQPX.js";
import "./chunk-VBPD3MAX.js";
import "./chunk-Z3PP4SLW.js";
import "./chunk-MVS4RVUO.js";
import "./chunk-PXICZPXV.js";
import "./chunk-OOTSAG75.js";
import "./chunk-DAHPVVCD.js";
import "./chunk-WWKLZ2P6.js";
import "./chunk-DVLEZ3TT.js";
import "./chunk-KEY3YQEB.js";
import {
  l
} from "./chunk-NLVGGH5B.js";
import "./chunk-BGBXZWYV.js";
import "./chunk-DMOCO4H5.js";
import "./chunk-UDRKO2UK.js";
import "./chunk-HJMMRQXJ.js";
import {
  U,
  a as a2
} from "./chunk-M44GXGOJ.js";
import "./chunk-LQMHYEKG.js";
import "./chunk-N7OQ4MFT.js";
import "./chunk-473S3KQM.js";
import "./chunk-HTVDNBEG.js";
import "./chunk-GALDA7CN.js";
import "./chunk-6TF3NI7G.js";
import "./chunk-QRPQOATM.js";
import "./chunk-OIICDJNU.js";
import "./chunk-FMIKVU3Q.js";
import "./chunk-R3PBZCGD.js";
import "./chunk-Z5I3WFZJ.js";
import "./chunk-DFCHXQYB.js";
import "./chunk-ODWYDPQK.js";
import "./chunk-6U5MFJUS.js";
import "./chunk-EQVY5WGD.js";
import "./chunk-5GPMO33J.js";
import "./chunk-GM5PCDS3.js";
import "./chunk-URLT4X25.js";
import "./chunk-SH72BZH2.js";
import "./chunk-CHITL6T5.js";
import "./chunk-OYOKYTYR.js";
import "./chunk-ONXOVX4W.js";
import "./chunk-TOB5EAWJ.js";
import "./chunk-NR66QFNF.js";
import "./chunk-FZD4GVUW.js";
import "./chunk-LANOLZOB.js";
import "./chunk-DK6LJVYU.js";
import "./chunk-U5RKVLEL.js";
import {
  p
} from "./chunk-RVKOLALF.js";
import "./chunk-253Z6EVN.js";
import "./chunk-FMWSBXS5.js";
import "./chunk-HXIOMN57.js";
import {
  a3 as a,
  c,
  o4 as o
} from "./chunk-JM4CKTH2.js";
import "./chunk-SZXJF3IE.js";
import "./chunk-HDFIZIZL.js";
import "./chunk-W3O3DL7T.js";
import "./chunk-SY7OJLZ7.js";
import "./chunk-IK45K3EN.js";
import "./chunk-BCADJITZ.js";
import "./chunk-PFC2RERS.js";
import "./chunk-RTVKY37F.js";
import {
  f2 as f
} from "./chunk-WARIPJQI.js";
import {
  n2 as n,
  r3 as r
} from "./chunk-6I475YAP.js";
import "./chunk-GLWFJLHD.js";
import {
  __decorate
} from "./chunk-HRD6PGVX.js";
import {
  __spreadProps,
  __spreadValues
} from "./chunk-653SOEEV.js";

// node_modules/@arcgis/core/layers/IntegratedMeshLayer.js
var P = class extends _2(l3(g(_(l2(e(s(s2(l(b))))))))) {
  constructor(...e2) {
    super(...e2), this.geometryType = "mesh", this.operationalLayerType = "IntegratedMeshLayer", this.type = "integrated-mesh", this.nodePages = null, this.materialDefinitions = null, this.textureSetDefinitions = null, this.geometryDefinitions = null, this.serviceUpdateTimeStamp = null, this.profile = "mesh-pyramids", this.modifications = null, this.path = null, this.definitionExpression = null;
  }
  initialize() {
    this.addHandles(a2(() => this.modifications, "after-changes", () => this.modifications = this.modifications, U));
  }
  normalizeCtorArgs(e2, t) {
    return "string" == typeof e2 ? __spreadValues({ url: e2 }, t) : e2;
  }
  readModifications(e2, t, o2) {
    this._modificationsSource = { url: p(e2, o2), context: o2 };
  }
  set elevationInfo(e2) {
    null != e2 && "absolute-height" !== e2.mode || this._set("elevationInfo", e2), this._validateElevationInfo(e2);
  }
  async load(e2) {
    return this.addResolvingPromise(this._doLoad(e2)), this;
  }
  async _doLoad(e2) {
    const t = e2?.signal;
    try {
      await this.loadFromPortal({ supportedTypes: ["Scene Service"] }, e2);
    } catch (o2) {
      f(o2);
    }
    if (await this._fetchService(t), null != this._modificationsSource) {
      const t2 = await p3.fromUrl(this._modificationsSource.url, this.spatialReference, e2);
      this.setAtOrigin("modifications", t2, this._modificationsSource.context.origin), this._modificationsSource = null;
    }
    await this._fetchIndexAndUpdateExtent(this.nodePages, t);
  }
  beforeSave() {
    if (null != this._modificationsSource) return this.load().then(() => {
    }, () => {
    });
  }
  async saveAs(e2, t) {
    return this._debouncedSaveOperations(1, __spreadProps(__spreadValues({}, t), { getTypeKeywords: () => this._getTypeKeywords(), portalItemLayerType: "integrated-mesh" }), e2);
  }
  async save() {
    const e2 = { getTypeKeywords: () => this._getTypeKeywords(), portalItemLayerType: "integrated-mesh" };
    return this._debouncedSaveOperations(0, e2);
  }
  validateLayer(e2) {
    if (e2.layerType && "IntegratedMesh" !== e2.layerType) throw new r("integrated-mesh-layer:layer-type-not-supported", "IntegratedMeshLayer does not support this layer type", { layerType: e2.layerType });
    if (isNaN(this.version.major) || isNaN(this.version.minor)) throw new r("layer:service-version-not-supported", "Service version is not supported.", { serviceVersion: this.version.versionString, supportedVersions: "1.x" });
    if (this.version.major > 1) throw new r("layer:service-version-too-new", "Service version is too new.", { serviceVersion: this.version.versionString, supportedVersions: "1.x" });
  }
  _getTypeKeywords() {
    return ["IntegratedMeshLayer"];
  }
  _validateElevationInfo(e2) {
    const t = "Integrated mesh layers";
    j(n.getLogger(this), w(t, "absolute-height", e2)), j(n.getLogger(this), R(t, e2));
  }
};
__decorate([a({ type: String, readOnly: true })], P.prototype, "geometryType", void 0), __decorate([a({ type: ["show", "hide"] })], P.prototype, "listMode", void 0), __decorate([a({ type: ["IntegratedMeshLayer"] })], P.prototype, "operationalLayerType", void 0), __decorate([a({ json: { read: false }, readOnly: true })], P.prototype, "type", void 0), __decorate([a({ type: p2, readOnly: true, clonable: false })], P.prototype, "nodePages", void 0), __decorate([a({ type: [a3], readOnly: true, clonable: false })], P.prototype, "materialDefinitions", void 0), __decorate([a({ type: [y], readOnly: true, clonable: false })], P.prototype, "textureSetDefinitions", void 0), __decorate([a({ type: [m2], readOnly: true, clonable: false })], P.prototype, "geometryDefinitions", void 0), __decorate([a({ readOnly: true })], P.prototype, "serviceUpdateTimeStamp", void 0), __decorate([a({ type: p3, clonable: (e2) => e2?.clone() ?? e2 }), j2({ origins: ["web-scene", "portal-item"], type: "resource", prefix: "modifications" })], P.prototype, "modifications", void 0), __decorate([o(["web-scene", "portal-item"], "modifications")], P.prototype, "readModifications", null), __decorate([a(m)], P.prototype, "elevationInfo", null), __decorate([a({ type: String, json: { origins: { "web-scene": { read: true, write: true }, "portal-item": { read: true, write: true } }, read: false } })], P.prototype, "path", void 0), P = __decorate([c("esri.layers.IntegratedMeshLayer")], P);
var U2 = P;
export {
  U2 as default
};
//# sourceMappingURL=IntegratedMeshLayer-3W7BSWU7.js.map

...
function a3(a8, l10) {
  ...
  if (!s11 && !f15) throw new r("heatmap:missing-color-buffer-float", "HeatmapRenderer requires the WebGL extension EXT_color_buffer_float or EXT_color_buffer_half_float or WEBGL_color_buffer_float.");
  if (!(u19 && _5 || f15)) throw new r("heatmap:missing-float-blend", "HeatmapRenderer requires the WebGL extension EXT_float_blend or EXT_color_buffer_half_float." + (_5 ? "" : " This device claims support for EXT_float_blend, but does not actually support it."));
  ...
  throw new r("heatmap:missing-hardware-support", "HeatmapRenderer requires WebGL extensions that allow it to render and blend to float or half float textures.");
}
...
var H4 = class extends B6 {
  ...
  _getDistanceRatio(t8, e9) {
    ...
    return o16.multiply(Xe(e9, new C2(0.25)).multiply(new C2(2))).add(new C2(1).subtract(o16).multiply(u(1)));
  }
  _getSDFAlpha(t8) {
  ...
  switch (o16.type) {
    ...
    case "heatmap":
      if (!g12()) {
        ...
        return n15().errorOnce(new r("webgl-missing-extension", `Missing WebGL2 requirements for Heatmap: ${r9}`)), false;
      }
  }
  return true;
}
...
function V3(a8) {
  ...
  if (a8.stops.some((e9) => e9.useMaxValue || e9.useMinValue)) return (i9, t9) => {
    const r9 = i9.statisticsByLevel.get(t9.key.level), u19 = a8.stops.map((i10) => ({ value: i10.useMaxValue ? r9?.get(a8.field)?.maxValue ?? 0 : i10.useMinValue ? r9?.get(a8.field)?.minValue ?? 0 : i10.value, size: i10.size ? u(i10.size) : c })).sort((e9, a9) => e9.value - a9.value), n17 = n14(u19, 8);
    return { values: n17.map(({ value: e9 }) => e9), sizes: n17.map(({ size: e9 }) => e9) };
  };
  const i8 = a8.stops.sort((e9, a9) => e9.value - a9.value), t8 = n14(i8, 8);
  return { values: t8.map(({ value: e9 }) => e9), sizes: t8.map(({ size: a9 }) => u(a9)) };
}
function f12(e9) {
  return (l10) => {
    const { state: s11 } = l10;
...
function S5(a8) {
  ...
  if ("object" == typeof s11 && "object" == typeof t8) return (a9, r9) => {
    const u19 = a9.state.scale, n17 = u(m10(u19, s11.stops)), o16 = u(m10(u19, t8.stops));
  ...
  return { minMaxValueAndSize: [i8, l10, u(s11), u(t8)] };
}
...
var o15 = class {
  constructor(e9) {
    this.updateTracking = new n2({ debugName: "FeatureCommandQueue" }), this.process = e9.process, this._queueProcessor = new u2({ concurrency: 1, process: async (e10) => {
...
//# sourceMappingURL=chunk-YNTZHNQQ.js.map

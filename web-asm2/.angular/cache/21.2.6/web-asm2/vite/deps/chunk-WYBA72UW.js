...
function g(e2) {
  ...
  const t2 = e2.map((e3) => {
    if (!Number.isFinite(e3.scale) || e3.scale <= 0) throw new r2("effect:invalid-scale", "scale must be finite and greater than 0", { stop: e3 });
    return { scale: e3.scale, effects: d2(e3.value) };
  });
  t2.sort((e3, t3) => t3.effects.length - e3.effects.length);
  for (let n = 0; n < t2.length - 1; n++) {
    if (!t(t2[n].effects, t2[n + 1].effects)) throw new r2("effect:interpolation-impossible", "Cannot interpolate by scale between 2 lists of mixed effects", { a: t2[n].effects, b: t2[n + 1].effects });
...
function d2(e2) {
  ...
  try {
    ...
    throw new r2("effect:invalid-syntax", "Invalid effect syntax", { value: e2, error: n });
  }
...
function y(e2) {
  ...
  throw new r2("effect:unknown-effect", `Effect '${e2.name}' is not supported`, { effect: e2 });
}
function w(e2) {
  let t2 = 1;
...
function b(e2) {
  ...
  for (const r3 of e2.parameters) if ("color" === r3.type) {
    if (t2.length && Object.freeze(t2), n) throw new r2("effect:type-error", "Accepts only one color", {});
    n = N(r3);
  } else {
    const e3 = k(r3);
    if (Object.isFrozen(t2)) throw new r2("effect:type-error", "<length> parameters not consecutive", { lengths: t2 });
    t2.push(e3), 3 === t2.length && C(e3, r3);
  }
  if (t2.length < 2 || t2.length > 3) throw new r2("effect:type-error", `Expected <length>{2,3}, Actual: <length>{${t2.length}}`, { lengths: t2 });
...
function E(e2, t2) {
  if (e2.length > t2) throw new r2("effect:type-error", `Function supports up to ${t2} parameters, Actual: ${e2.length}`, { parameters: e2 });
}
function F(e2) {
...
function C(e2, t2) {
  if (e2 < 0) throw new r2("effect:type-error", `Negative values are not allowed, Actual: ${e2}`, { term: t2 });
}
function j(e2) {
  if ("quantity" !== e2.type || null !== e2.unit) throw new r2("effect:type-error", `Expected <double>, Actual: ${F(e2)}`, { term: e2 });
}
function P(e2) {
  if ("quantity" !== e2.type || null !== e2.unit && "%" !== e2.unit) throw new r2("effect:type-error", `Expected <double> or <percentage>, Actual: ${F(e2)}`, { term: e2 });
}
var R = { deg: 1, grad: 0.9, rad: 180 / Math.PI, turn: 360 };
function S(e2) {
  if ("quantity" !== e2.type || !(0 === e2.value && null === e2.unit || e2.unit && null != R[e2.unit])) throw new r2("effect:type-error", `Expected <angle>, Actual: ${F(e2)}`, { term: e2 });
}
var q = { px: 1, cm: 96 / 2.54, mm: 96 / 2.54 / 10, in: 96, pc: 16, pt: 96 / 72 };
function M(e2) {
  if ("quantity" !== e2.type || !(0 === e2.value && null === e2.unit || e2.unit && null != q[e2.unit])) throw new r2("effect:type-error", `Expected <length>, Actual: ${F(e2)}`, { term: e2 });
}
function z(e2) {
  P(e2);
  const t2 = e2.value;
...
function T(e2) {
  if (!r(e2)) throw new r2("effect:unknown-color", `color '${e2}' isn't valid`, { namedColor: e2 });
  return l(e2);
}
var U = /^rgba?/i;
var D = /^hsla?/i;
function L(e2) {
  ...
  throw new r2("effect:syntax-error", `Invalid color function '${e2.name}'`, { colorFunction: e2 });
}

export {
  g
};
//# sourceMappingURL=chunk-WYBA72UW.js.map

...
async function I(e2, r2) {
  const t = await E(e2, r2, async () => {
    try {
      if (!e2.url) return [];
      const { serverUrl: t2 } = await s3(e2.url, { sceneLayerItem: e2 }), s4 = await r2.fetchServiceMetadata(t2);
      return s4?.tables ?? [];
    } catch {
      return [];
    }
  ...
  if ("object" == typeof t) {
    ...
    const c2 = await r2.fetchServiceMetadata(n2, { customParameters: await r2.fetchCustomParameters(e2, (e3) => s2(e3)?.customParameters) });
...
async function M(e2) {
  await e2.load();
  const a2 = s(e2, "Map Notes"), r2 = s(e2, "Markup");
  if (a2 || r2) return { className: "MapNotesLayer" };
  if (s(e2, "Route Layer")) return { className: "RouteLayer" };
...
async function E(e2, a2, t) {
  ...
  if (/\/\d+$/.test(s4)) {
    if (i2) {
      const t2 = await a2.fetchServiceMetadata(s4, { customParameters: await a2.fetchCustomParameters(e2, (e3) => s2(e3)?.customParameters) });
...
//# sourceMappingURL=chunk-L6EDOPXJ.js.map

...
var A = class _A {
  ...
  buildOffset() {
    ...
    for (let o = 0; o < s2; o++) {
      ...
      let a2, f2, P2 = 0;
      {
        ...
        a2 = Math.atan2(s3, t3), f2 = Math.atan2(_, e2), this.m_a1 = a2, this.m_a2 = f2, 0 === o && (i = a2, n = f2), P2 = mi.orientationRobust(h.asPoint2D(), r.asPoint2D(), m.asPoint2D());
      }
      const d = f2;
      if (f2 < a2 && (f2 += Kt), !(Math.abs(Math.abs(a2 - f2) - Math.PI) < 1e-8)) if (P2 * this.m_distance > 0) if (1 === this.m_joins || 2 === this.m_joins) {
        ...
        if (1 !== this.m_joins) {
          ...
          if (2 === this.m_joins) {
            ...
            const u = Math.abs(this.m_distance / Math.sin(0.5 * Math.acos(_))), P3 = Math.abs(this.m_miterLimit * this.m_distance);
            if (u > P3) {
              ...
              r2.setSub(i3, n3);
              const m2 = new mi();
              m2.setScaleAdd(P3 / r2.length(), r2, n3);
              const _2 = Math.sqrt(u * u - this.m_distance * this.m_distance), d3 = (u - P3) * Math.abs(this.m_distance) / _2;
              this.m_distance > 0 ? r2.rightPerpendicularThis() : r2.leftPerpendicularThis(), r2.scale(d3 / r2.length());
              const l3 = new mi();
              l3.setAdd(m2, r2);
              const y2 = new mi();
            ...
            const d2 = 0.5 * (f2 - a2), l2 = this.m_distance / Math.abs(Math.sin(d2));
            _A.buildPoint(h, l2, 0.5 * (a2 + f2), t2), this.addPointEx(t2, o);
            continue;
          }
          {
            let s3;
            if (f2 = d, this.m_distance > 0 ? (f2 > a2 && (f2 -= Kt), s3 = a2 - f2 < Wt) : (f2 < a2 && (f2 += Kt), s3 = f2 - a2 < Wt), s3) {
  ...
  handleClosedIntersection(t2, s2) {
    ...
    if (((i.x - e.x) * (o.y - n.y) - (i.y - e.y) * (o.x - n.x)) * this.m_distance < 0) {
      if (this.findIntersection(e, i, n, o, h) && !h.bAtExistingPt) {
        const r = 1e-8, m = Math.sqrt((i.x - e.x) * (i.x - e.x) + (i.y - e.y) * (i.y - e.y)), a2 = (i.x - e.x) / m, f2 = (i.y - e.y) / m, c = Math.sqrt((o.x - n.x) * (o.x - n.x) + (o.y - n.y) * (o.y - n.y)), _ = (o.x - n.x) / c, u = (o.y - n.y) / c;
        let P2 = false;
        const d = h.pt.clone();
        d.x += (a2 + _) * r, d.y += (f2 + u) * r;
        let l2 = n, y2 = h.pt, x2 = this.m_offsetPts[t2].m_prev;
        for (; y2.y > d.y != l2.y > d.y && d.x < (l2.x - y2.x) * (d.y - y2.y) / (l2.y - y2.y) + y2.x && (P2 = !P2), l2 = y2, x2 = this.m_offsetPts[x2].m_next, x2 !== s2; ) y2 = this.m_offsetPts[x2];
        if (P2) return -1;
  ...
  offsetMultiPath(t2) {
    const s2 = Bh(this.m_inputGeometry, 0, this.m_tolerance, 0, this.m_progressTracker, 12e3), e = s2.querySegmentIterator();
    e.resetToFirstPath();
    let i = -1;
...
var v = class extends s {
  ...
  offset(t2) {
    let s2;
    return j(t2), s2 = this.m_flattenError <= 0 ? zt(this.m_spatialReference, t2, true).total() : this.m_flattenError, l(t2, this.m_distance, this.m_joins, this.m_miterLimit, s2, this.m_progressTracker);
  }
};
var G = new E();
...
function j2(s2, e, i, n, o, h) {
  const r = G.executeMany(new t(s2), e, i, n, o, h, null);
  return Array.from(r);
}
...
//# sourceMappingURL=chunk-LFAUN6PY.js.map

...
var w = class extends S(I) {
  *graphicsViews() {
  ...
  highlight(e2, a2) {
    let o;
    "number" == typeof e2 ? o = [e2] : e2 instanceof j ? o = [e2.uid] : Array.isArray(e2) && e2.length > 0 ? o = "number" == typeof e2[0] ? e2 : e2.map((e3) => e3 && e3.uid) : q.isCollection(e2) && (o = e2.map((e3) => e3 && e3.uid).toArray());
    const h2 = o?.filter(N);
  ...
  attach() {
    ...
    if (null != s && s.length) {
      ...
      for (const t of s) {
        const s2 = new i(this.view.featuresTilingScheme), r2 = new O({ view: e2, graphics: t.source, renderer: t.renderer, requestUpdateCallback: i2, container: s2 });
    ...
    } else null != this.layer.sublayers && this.addHandles(a(() => this.layer.sublayers, "change", () => this._createGraphicsViews(), { onListenerAdd: () => this._createGraphicsViews(), onListenerRemove: () => this._destroyGraphicsViews() }), u);
  }
  detach() {
  ...
  _createGraphicsViews() {
    ...
    for (const t of this.layer.sublayers) {
      ...
      const h2 = new O({ view: i2, graphics: t.graphics, requestUpdateCallback: s, container: r2 });
...
//# sourceMappingURL=MapNotesLayerView2D-VOYZEUG7.js.map
